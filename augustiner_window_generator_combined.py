...
import argparse
import os
from svg_gen_window import generate_window_svg as _generate_window_svg, generate_windows_svg, WindowSpec
...
def generate_svg(svg_dir: str, combined_output: str | None = None, columns: int = 3):
    """Write the individual windows and, optionally, one labelled SVG sheet."""
    ...
    def generate_window_svg(spec: WindowSpec) -> str:
        ...
        return _generate_window_svg(spec)
    ...
    if combined_output:
        combined_path = os.path.join(svg_dir, combined_output)
        with open(combined_path, "w", encoding="utf-8") as f:
            f.write(generate_windows_svg(all_specs, columns=columns))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--combined-output", nargs="?", const="Augustiner_all_windows.svg",
        help="Also write all labelled windows to this SVG file (default: Augustiner_all_windows.svg).",
    )
    parser.add_argument("--columns", type=int, default=3, help="Columns in the combined SVG (default: 3).")
    args = parser.parse_args()
    generate_svg(args.svg_dir, combined_output=args.combined_output, columns=args.columns)
